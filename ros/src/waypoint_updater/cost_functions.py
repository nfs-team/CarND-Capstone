...
# COST FUNCTIONS
def min_velocity_cost(a, max_v, T):
    s = a
...
def max_velocity_cost(a, max_v, T):
    s = a
    s_dot = differentiate(s)
    v = to_equation(s_dot)
    all_vels = [v(float(T) / 500 * i) for i in range(500)]
    max_vel = max(all_vels, key=abs)
    if abs(max_vel) > max_v:
        return 1
    else:
        return 0

def efficiency_cost(a, max_v, T):
    """
...
def total_accel_cost(a, max_v, T):
    s = a
    s_dot = differentiate(s)
